...
#include "DX11RendererImmediateImpl.h"
#include "IActor.h"
...
void graphics::dx11::renderer::DX11RendererImmediateImpl::Draw(const float x, const float y, const float width, const float height, const float r, const float g, const float b, const float a, const float rot)
{
	DX11Core& rCore = DX11Core::Instance();
...
#pragma region // update vertex translate transform. this will be the position. 
	m_UpdateConstantBuffer.vertex.translate.x = (-m_D3DViewPort.Width + width) / 2 + x;
	m_UpdateConstantBuffer.vertex.translate.y = (m_D3DViewPort.Height - height) / 2 + -y;
#pragma endregion

#pragma region // update rotation
	m_UpdateConstantBuffer.Misc.rotate = rot;
#pragma endregion

#pragma region // update color
	m_UpdateConstantBuffer.color.r = r;
	m_UpdateConstantBuffer.color.g = g;
	m_UpdateConstantBuffer.color.b = b;
	m_UpdateConstantBuffer.color.a = a;
#pragma endregion
...
#pragma region // update color
	m_UpdateConstantBuffer.color.r = red;
	m_UpdateConstantBuffer.color.g = green;
	m_UpdateConstantBuffer.color.b = blue;
	m_UpdateConstantBuffer.color.a = alpha;
#pragma endregion
...
{
	DrawRenderable(actor, actor.GetTransform().GetPosition(), actor.GetSize(), tint, rotation);
}
