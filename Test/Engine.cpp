...
void engine::Engine::PrintText(const std::string& text, spatial::PosF position, graphics::ColorF color)
{
	m_Renderer->DrawText(m_fontAtlas, text, position.x, position.y, color.red, color.green, color.blue, color.alpha);
}
...
void engine::Engine::OnWindowCreate(void* hWnd)
{
	...
	LOG("Using canvas type: " << m_canvas->GetTypeName());
	

	// create sprite renderer
	m_Renderer = graphics::factory::RendererFactory::Create(environmentConfig.Get("RenderMode"));
	if(!m_Renderer)
	{
	...
	LOG("Using sprite renderer mode: " << environmentConfig.Get("RenderMode"));

	// create utility font atlas
	m_fontAtlas = std::make_shared<graphics::resource::FontAtlas>(graphics::factory::TextureFactory::Create());
	m_fontAtlas->Initialize("Comic Sans MS", 24);
...
void engine::Engine::OnWindowSize(size_t width, size_t height)
{
	...
	m_canvas->Resize(static_cast<uint32_t>(width), static_cast<uint32_t>(height));
	OnResize(width, height);
}

void engine::Engine::DrawLineSegment(
...
{
	graphics::primitives::DrawLineSegment(
		*m_Renderer,
		start,
		end,
		color,
...
void engine::Engine::DrawCircleOutline(
...
{
	graphics::primitives::DrawCircleOutline(
		*m_Renderer,
		center,
		radius,
		color,
