...
#include "DX11RendererBatchImpl.h"
#include "IActor.h"
...
void graphics::dx11::renderer::DX11RendererBatchImpl::Draw(const float x, const float y, const float width, const float height, const float r, const float g, const float b, const float a, const float rot)
{
...
#pragma region // update vertex translate transform. this will be the position. 
	m_UpdateConstantBuffer.vertex[m_nCurrSpriteCount].translate.x = (-m_D3DViewPort.Width + width) / 2 + x;
	m_UpdateConstantBuffer.vertex[m_nCurrSpriteCount].translate.y = (m_D3DViewPort.Height - height) / 2 + -y;
#pragma endregion

#pragma region // update rotation
	m_UpdateConstantBuffer.Misc[m_nCurrSpriteCount].rotate = rot;
#pragma endregion
...
#pragma region // update color
	m_UpdateConstantBuffer.color[m_nCurrSpriteCount].r = r;
	m_UpdateConstantBuffer.color[m_nCurrSpriteCount].g = g;
	m_UpdateConstantBuffer.color[m_nCurrSpriteCount].b = b;
	m_UpdateConstantBuffer.color[m_nCurrSpriteCount].a = a;
#pragma endregion

#pragma region // set view clip
#if SPRITEBATCH_USESHADERWITHRECTVIEW
	m_clipRegion = { 50, 50, 800, 700 };
...
)
{
	...
	{
		// draw the char
		DrawChar(font, c, xCurr, y, red, green, blue, alpha, 0);
...
#pragma region // set color to white as it will be multiplied with texture color and we don't want to change the texture color
	m_UpdateConstantBuffer.color[m_nCurrSpriteCount].r = red;
	m_UpdateConstantBuffer.color[m_nCurrSpriteCount].g = green;
	m_UpdateConstantBuffer.color[m_nCurrSpriteCount].b = blue;
	m_UpdateConstantBuffer.color[m_nCurrSpriteCount].a = alpha;
#pragma endregion
...
{
	DrawRenderable(
		actor, actor.GetTransform().GetPosition() + actor.GetAnimator().GetCurrentFrame()->element.GetRenderOffset(),
		actor.GetSize(), 
		tint, 
		rotation
	);
